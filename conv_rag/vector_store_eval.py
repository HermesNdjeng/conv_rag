...
logger: Logger = setup_logger("eval")


def evaluate_vector_stores(
    test_queries: list[str], embedding_model: Any | None = None
) -> pd.DataFrame:
    """
    ...
    for i, query in enumerate(tqdm(test_queries, desc="Évaluation des vector stores")):
        ...
        start_time_faiss: float = time.time()
        faiss_result = faiss_retriever.retrieve(query)
        faiss_time: float = time.time() - start_time_faiss

        start_time_chroma: float = time.time()
        chroma_result = chroma_retriever.retrieve(query)
        chroma_time: float = time.time() - start_time_chroma
        ...
        overlap: int = len(
            {doc.page_content for doc in faiss_docs} & {doc.page_content for doc in chroma_docs}
        )
        overlap_percent: float = overlap / 8 * 100 if faiss_docs and chroma_docs else 0

        logger.debug(
            f"Requête: '{query}' | Similarité FAISS: {faiss_avg_similarity:.4f} | "
    ...
    logger.info(
        f"Temps moyen FAISS: {result_df['faiss_time'].mean():.4f}s | "
        f"Temps moyen Chroma: {result_df['chroma_time'].mean():.4f}s"
    )
    logger.info(
        f"Similarité moyenne FAISS: {result_df['faiss_avg_similarity'].mean():.4f} | "
...
def evaluate_answer_quality(
    test_questions: list[str], generator_config: GeneratorConfig
) -> pd.DataFrame:
    """
    ...
    for i, question in enumerate(tqdm(test_questions, desc="Évaluation des réponses")):
        ...
        faiss_generator: RAGGenerator = RAGGenerator(
            config=generator_config, retriever=faiss_retriever
        )
        ...
        chroma_generator: RAGGenerator = RAGGenerator(
            config=generator_config, retriever=chroma_retriever
        )
        ...
        faiss_token_count: int | float = (
            faiss_result.token_usage.get("total_tokens", 0) if faiss_result.token_usage else 0
        )
        chroma_token_count: int | float = (
            chroma_result.token_usage.get("total_tokens", 0) if chroma_result.token_usage else 0
        )
    ...
    logger.info(
        f"Consommation moyenne de tokens - FAISS: "
        f"{result_df['faiss_token_count'].mean():.1f} | "
        f"Chroma: {result_df['chroma_token_count'].mean():.1f}"
    )
    logger.info(
        f"Pourcentage de réponses identiques: " f"{(result_df['answer_match'].mean() * 100):.1f}%"
    )
...
def evaluate_chunk_relevance(
    test_queries: list[str], ground_truth: Any | None = None
) -> pd.DataFrame:
    """
    ...
    faiss_retriever: DocumentRetriever = DocumentRetriever(
        config=RetrievalConfig(index_type="faiss", top_k=5)
    )
    chroma_retriever: DocumentRetriever = DocumentRetriever(
        config=RetrievalConfig(index_type="chroma", top_k=5)
    )

    # Créer un évaluateur LLM
    logger.info("Initialisation du modèle évaluateur GPT-4...")
    evaluator: ChatOpenAI = ChatOpenAI(model="gpt-4", temperature=0)

    eval_prompt: PromptTemplate = PromptTemplate(
        template="""
        [INSTRUCTION]
    ...
    for i, query in enumerate(tqdm(test_queries, desc="Évaluation de la pertinence")):
        logger.debug(
            f"Évaluation de la pertinence pour la requête {i+1}/{len(test_queries)}: '{query}'"
        )
        ...
        for j, chunk in enumerate(faiss_chunks):
            ...
            result: str = evaluator.predict(
                eval_prompt.format(query=query, chunk=chunk.page_content)
            )
            try:
                score: int = int(result.strip())
                faiss_scores.append(score)
                logger.debug(f"Score de pertinence FAISS {j+1}: {score}/10")
            except ValueError:
                logger.warning(
                    f"Impossible de convertir le score pour le chunk FAISS {j+1}: {result}"
                )
                faiss_scores.append(0)

        chroma_scores: list[int] = []
        ...
        for j, chunk in enumerate(chroma_chunks):
            ...
            try:
                ...
                logger.warning(
                    f"Impossible de convertir le score pour le chunk Chroma {j+1}: {result}"
                )
        ...
        logger.info(
            f"Requête: '{query}' | Pertinence moyenne FAISS: {faiss_avg:.2f}/10 | "
    ...
    logger.info(
        f"Pertinence moyenne - FAISS: {result_df['faiss_relevance'].mean():.2f}/10 | "
...
def plot_comparison(
    df: pd.DataFrame, metric_cols: list[str], title: str, output_dir: str = "results"
) -> None:
    """
    ...
    df[metric_cols].mean().plot(kind="bar", color=["#1f77b4", "#ff7f0e"])  # type: ignore
    plt.title(title, fontsize=16)  # type: ignore
    plt.ylabel("Score", fontsize=14)  # type: ignore
    plt.xticks(fontsize=12)  # type: ignore
    plt.yticks(fontsize=12)  # type: ignore
    plt.grid(axis="y", linestyle="--", alpha=0.7)  # type: ignore

    # Ajouter les valeurs sur les barres
    for i, v in enumerate(df[metric_cols].mean()):  # type: ignore
        plt.text(i, v + 0.01, f"{v:.4f}", ha="center", fontsize=11)  # type: ignore

    plt.tight_layout()

    output_file: str = os.path.join(output_dir, f"{title.lower().replace(' ', '_')}.png")
...
def main() -> int:
    ...
    parser: argparse.ArgumentParser = argparse.ArgumentParser(
        description="Évaluation des performances de FAISS vs Chroma"
    )
    parser.add_argument(
        "--output-dir", default="results", help="Répertoire de sortie pour les résultats"
    )
    parser.add_argument(
        "--similarity-only",
        action="store_true",
        help="Exécuter uniquement l'évaluation de similarité",
    )
    parser.add_argument(
        "--quality-only",
        action="store_true",
        help="Exécuter uniquement l'évaluation de qualité des réponses",
    )
    parser.add_argument(
        "--relevance-only",
        action="store_true",
        help="Exécuter uniquement l'évaluation de pertinence",
    )
    args: argparse.Namespace = parser.parse_args()

    # Créer le répertoire de sortie
    timestamp: str = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir: str = os.path.join(args.output_dir, f"eval_{timestamp}")
    os.makedirs(output_dir, exist_ok=True)

    logger.info("Démarrage de l'évaluation des vector stores (FAISS vs Chroma)")
    ...
    relevance_results: pd.DataFrame | None = None
    ...
    if args.similarity_only or not (args.quality_only or args.relevance_only):
        ...
        plot_comparison(
            similarity_results,
            ["faiss_avg_similarity", "chroma_avg_similarity"],
            "Similarité Moyenne des Chunks",
            output_dir,
        )
        plot_comparison(
            similarity_results,
            ["faiss_time", "chroma_time"],
            "Temps de Récupération (secondes)",
            output_dir,
        )
        plot_comparison(
            similarity_results,
    ...
    if args.quality_only or not (args.similarity_only or args.relevance_only):
        ...
        quality_results = evaluate_answer_quality(
            test_questions[:3], generator_config
        )  # Limité à 3 pour l'efficacité
        ...
        plot_comparison(
            quality_results,
            ["faiss_token_count", "chroma_token_count"],
    ...
    if args.relevance_only or not (args.similarity_only or args.quality_only):
        ...
        relevance_results = evaluate_chunk_relevance(
            test_questions[:3]
        )  # Limité à 3 pour économiser GPT-4
        relevance_results.to_csv(os.path.join(output_dir, "relevance_results.csv"), index=False)

        plot_comparison(
            relevance_results,
            ["faiss_relevance", "chroma_relevance"],
            "Pertinence Moyenne des Chunks",
            output_dir,
        )
        plot_comparison(
            relevance_results,
            ["faiss_max_relevance", "chroma_max_relevance"],
            "Pertinence Max des Chunks",
            output_dir,
        )
    ...
    with open(os.path.join(output_dir, "evaluation_summary.txt"), "w") as f:
        ...
        if similarity_results is not None:
            ...
            f.write(
                f"Similarité moyenne FAISS: "
                f"{similarity_results['faiss_avg_similarity'].mean():.4f}\n"
            )
            f.write(
                f"Similarité moyenne Chroma: "
                f"{similarity_results['chroma_avg_similarity'].mean():.4f}\n"
            )
        ...
        if relevance_results is not None:
            ...
            f.write(
                f"Pertinence moyenne FAISS: "
                f"{relevance_results['faiss_relevance'].mean():.2f}/10\n"
            )
            f.write(
                f"Pertinence moyenne Chroma: "
                f"{relevance_results['chroma_relevance'].mean():.2f}/10\n"
            )
            f.write(
                f"Pertinence maximale moyenne FAISS: "
                f"{relevance_results['faiss_max_relevance'].mean():.2f}/10\n"
            )
            f.write(
                f"Pertinence maximale moyenne Chroma: "
                f"{relevance_results['chroma_max_relevance'].mean():.2f}/10\n\n"
            )
        ...
        if quality_results is not None:
            ...
            f.write(
                f"Pourcentage de réponses identiques: "
                f"{(quality_results['answer_match'].mean() * 100):.1f}%\n"
            )
