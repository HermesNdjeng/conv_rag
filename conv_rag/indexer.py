...
class IndexerConfig(BaseModel):
    ...
    index_path: str = Field(default="data/indexes", description="Directory to store vector indexes")
...
class DocumentIndexer:
    ...
    def create_faiss_index(
        self, documents: list[Document], index_name: str = "um_nyobe_index"
    ) -> IndexResult:
        """
        ...
        return IndexResult(index_type="faiss", document_count=len(documents), index_path=save_path)

    def create_chroma_index(
        self, documents: list[Document], index_name: str = "um_nyobe_chroma"
    ) -> IndexResult:
        ...
        vector_store = Chroma.from_documents(  # type: ignore
            documents=documents, embedding=self.embeddings, persist_directory=save_path
        )
        vector_store.persist()

        return IndexResult(index_type="chroma", document_count=len(documents), index_path=save_path)

    def load_faiss_index(self, index_name: str = "um_nyobe_index") -> FAISS | None:
    ...
    def load_chroma_index(self, index_name: str = "um_nyobe_chroma") -> Chroma | None:
