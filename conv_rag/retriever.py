...
class RetrievalConfig(BaseModel):
    ...
    index_type: Literal["faiss", "chroma"] = Field(
        default="faiss", description="Type of vector index to use for retrieval"
    )
    index_name: str | None = Field(default=None, description="Name of the index to retrieve from")
    top_k: int = Field(default=5, description="Number of documents to retrieve for each query")
    score_threshold: float | None = Field(
        default=0.3, description="Minimum similarity score threshold (0-1)"
    )
...
class DocumentRetriever:
    ...
    def __init__(
        ...
        indexer: DocumentIndexer | IndexerConfig | None = None,
    ):
        """
        ...
        logger.info(
            f"Initializing retriever with {self.config.index_type} index: {self.config.index_name}"
        )
    ...
    def _load_vector_store(self):
        ...
        if self.config.index_type == "faiss":
            store = (
                self.indexer.load_faiss_index(self.config.index_name)  # type: ignore
            ...
            store = (
                self.indexer.load_chroma_index(self.config.index_name)  # type: ignore
                if self.config.index_name
                else None
            )
            if store:
    ...
    def retrieve(self, query: str) -> RetrievalResult:
        ...
        if self.config.index_type == "faiss":
            docs_with_scores = self.vector_store.similarity_search_with_score(  # type: ignore
                query, k=self.config.top_k
            )
            # FAISS returns (doc, score) tuples
            documents = [doc for doc, score in docs_with_scores]  # type: ignore
            scores = [1.0 - score for doc, score in docs_with_scores]  # type: ignore # Convert distance to similarity
            ...
            documents = [doc for doc, score in docs_with_scores]  # type: ignore
            scores = [score for doc, score in docs_with_scores]  # type: ignore
        ...
        if self.config.score_threshold is not None:
            ...
            if filtered_results:
                ...
                logger.warning(
                    f"No documents met the similarity threshold of {self.config.score_threshold}"
                )

        # Logs détaillés pour chaque chunk récupéré
        logger.info(
            f"===== Retrieved {len(documents)} chunks "
            f"(top_k={self.config.top_k}, threshold={self.config.score_threshold}) ====="
        )
        for i, (doc, score) in enumerate(zip(documents, scores, strict=False)):
            source = doc.metadata.get("source", "Unknown source")  # type: ignore
            page = doc.metadata.get("page", "Unknown page")  # type: ignore
            word_count = len(doc.page_content.split())
            logger.info(
                f"CHUNK {i+1}: Score={score:.4f}, Source={source}, Page={page}, Words={word_count}"
            )
