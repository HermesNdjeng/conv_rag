...
from retriever import DocumentRetriever, RetrievalConfig, RetrievalResult
...
def index_documents():
    ...
    logger.info(
        f"Loaded {load_result.original_count} documents and split "
        f"into {load_result.chunk_count} chunks"
    )
    ...
    if load_result.chunks:
        ...
        logger.info(
            f"Created FAISS index with {faiss_result.document_count} documents"
            f" at {faiss_result.index_path}"
        )
        logger.info(
            f"Created Chroma index with {chroma_result.document_count} documents"
            "at {chroma_result.index_path}"
        )
...
def index_from_ocr():
    ...
    # Initialize the indexer with a French-optimized embedding model
    indexer_config = IndexerConfig(
        embedding_model_name="dangvantuan/sentence-camembert-base", index_path="data/indexes"
    )
    indexer = DocumentIndexer(config=indexer_config)
    ...
    logger.info(
        f"Created FAISS index with {faiss_result.document_count} documents at"
        f"{faiss_result.index_path}"
    )
    logger.info(
        f"Created Chroma index with {chroma_result.document_count} documents at"
        f"{chroma_result.index_path}"
    )

    return indexer, True


def display_random_chunks(chunks: list[Document], num_chunks: int = 6):
    ...
    for i, chunk in enumerate(selected_chunks):
        ...
        logger.info(
            f"Length: {len(chunk.page_content)} characters, {len(chunk.page_content.split())} words"
        )
...
def test_retrieval(indexer: DocumentIndexer | IndexerConfig | None):
    ...
    retrieval_config = RetrievalConfig(
        index_type="faiss", index_name="um_nyobe_index", top_k=3, score_threshold=0.5
    )
    ...
    for query in test_queries:
        logger.info(f"\n--- Testing query: {query} ---")
        result: RetrievalResult = retriever.retrieve(query)
...
def test_generation(indexer: DocumentIndexer | IndexerConfig | None = None):
    ...
    # Initialize the retriever
    retrieval_config = RetrievalConfig(
        index_type="faiss", index_name="um_nyobe_index", top_k=4, score_threshold=0.5
    )
    retriever = DocumentRetriever(config=retrieval_config, indexer=indexer)

    # Initialize the generator
    generator_config = GeneratorConfig(
        model_name="gpt-3.5-turbo", temperature=0.7, max_tokens=512, track_token_usage=True
    )
    ...
    for query in test_queries:
        ...
        if result.token_usage:
            ...
            logger.info(f"Token usage: {total_tokens} tokens, ${cost:.5f}")
