...
logger: Logger = setup_logger("conversation")
...
class Message(BaseModel):
    ...
    metadata: dict[str, Any] | None = Field(
        default=None, description="Additional metadata about the message"
    )
...
class ConversationManager:
    ...
    def __init__(self, system_message: str | None = None) -> None:
        ...
        self.memory: ConversationBufferMemory = ConversationBufferMemory(return_messages=True)
    ...
    def add_assistant_message(self, message: str, metadata: dict[str, Any] | None = None) -> None:
        """
        Add an assistant message to the conversation.

        Args:
            message: Assistant's message content
            metadata: Optional metadata (e.g., source docs, token usage)
        """
