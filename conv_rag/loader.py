...
class LoaderConfig(BaseModel):
    ...
    chunk_overlap: int = Field(
        default=200, description="Overlap between chunks for context preservation"
    )
    ocr_enabled: bool = Field(default=True, description="Whether to use OCR for scanned documents")
    ocr_output_dir: str | None = Field(
        default="data/ocr", description="Directory to save OCR'd text files"
    )
...
class DocumentLoader:
    def __init__(self, config: LoaderConfig | None = None):
        ...
        logger.info(
            f"Initializing document loader with chunk size="
            f"{self.config.chunk_size}, overlap={self.config.chunk_overlap}"
        )
